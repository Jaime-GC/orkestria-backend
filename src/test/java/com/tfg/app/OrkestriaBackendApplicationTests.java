...
class OrkestriaBackendApplicationTests {
    ...
    void resourceModuleFlow() {
        ...
        // aquí anidamos el objeto parent con su id, no parentId
        ...
        // 3) Crear recurso en ese grupo (anidando en "group")
        Map<String,Object> item = new LinkedHashMap<>();
        item.put("name", "Sala A");
        item.put("group", Map.of("id", pId));  // anidamos group, no groupId
        ResponseEntity<Map> iRes = restTemplate.postForEntity(
            "/api/resource-items",
            new HttpEntity<>(item, jsonHeaders()),
            Map.class
        );
        ...
        ResponseEntity<Map> ok = restTemplate.postForEntity(
            "/api/resource-items/" + iId + "/reservations",
        ...
        ResponseEntity<List> avail = restTemplate.exchange(
            "/api/resource-items/" + iId + "/availability?from=" + start + "&to=" + end,
            HttpMethod.GET,
        ...
        ResponseEntity<Map> uRes = restTemplate.exchange(
            "/api/resource-items/" + iId + "/reservations/" + resId,
            HttpMethod.PUT,
